            ...
            {items.map((item) => (
                <div key={item?.card?.info?.id} className="flex justify-between items-center">
                    <div className="w-9/12 text-left border-b-2 border-gray-300 p-2 m-2">
                        <span className="text-lg dark:text-white font-noto-sans mb-2">{item?.card?.info?.name}</span>
                    ...
                    <div className="w-[160px] h-[110px] p-[2px] relative object-cover overflow-hidden"> 
                        ...
                        <button onClick={()=>handleAddToCart(item)} className="absolute bottom-[-10px] left-1/2 transform -translate-x-1/2 px-10 hover:bg-gray-200 hover:scale-95 transition-all delay-100 rounded-lg text-lg tracking-widest font-reddit-mono font-extrabold pb-2 bg-white text-[#1BA672]">ADD</button>
                    </div>
                </div>
            ))}
        </div>
